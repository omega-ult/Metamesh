...
public class RoundedBox : PrimitiveBase
{
    ...
    protected override void ReshapeMesh()
    {
        ...
        // 更新每个面的顶点
        UpdatePlaneVertices(vertices, normals, 0, math.float4( 1, 0, 0, Width), math.float4(0,  1, 0, Height), math.float3(0, 0, -0.5f * Depth));
        UpdatePlaneVertices(vertices, normals, 1, math.float4(-1, 0, 0, Width), math.float4(0,  1, 0, Height), math.float3(0, 0,  0.5f * Depth));
        UpdatePlaneVertices(vertices, normals, 2, math.float4(0, 0,  1, Depth), math.float4(0, -1, 0, Height), math.float3(-0.5f * Width, 0, 0));
        UpdatePlaneVertices(vertices, normals, 3, math.float4(0, 0, -1, Depth), math.float4(0, -1, 0, Height), math.float3( 0.5f * Width, 0, 0));
        UpdatePlaneVertices(vertices, normals, 4, math.float4( 1, 0, 0, Width), math.float4(0, 0, -1, Depth), math.float3(0, -0.5f * Height, 0));
        UpdatePlaneVertices(vertices, normals, 5, math.float4(-1, 0, 0, Width), math.float4(0, 0, -1, Depth), math.float3(0,  0.5f * Height, 0));
    ...
    // 更新单个面的顶点和法线
    private void UpdatePlaneVertices(Vector3[] vertices, Vector3[] normals, int planeIndex, float4 ax, float4 ay, float3 offs)
    {
        var vc_edge = 2 + Divisions * 2;
